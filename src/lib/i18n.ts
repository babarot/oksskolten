...
const dict = {
  ...
  'notifications.translateEnabled': { ja: '本文プレビューを簡体字中国語に自動翻訳', en: 'Auto-translate body preview to Simplified Chinese' },
  'notifications.translateEnabledHint': { ja: '本文プレビューが簡体字中国語ではない場合のみ翻訳します。タイトルや画像はそのまま送信されます。', en: 'Only translate when the body preview is not already Simplified Chinese. Titles and images stay unchanged.' },
  ...
  'notifications.previewBody': { ja: 'タイトル: ${feedName} · ${count} 条\n各記事: 原文リンク / 分単位の公開時刻 / 正文原文 1 行 / 中文译文 1 行（仅在需要翻译时）/ 最大 3 枚の画像', en: 'Title: ${feedName} · ${count}\nEach article: source link / minute-level publish time / original body line / Chinese translation line when needed / up to 3 images' },
  ...
  'settings.fetchSchedule': { ja: 'フィード取得間隔', en: 'Feed Fetch Interval' },
  'settings.fetchScheduleDesc': { ja: 'RSS フィードのバックグラウンド取得に使う最小間隔を設定します', en: 'Configure the minimum interval used for background RSS feed fetches' },
  'settings.fetchScheduleMinInterval': { ja: '最小取得間隔', en: 'Minimum fetch interval' },
  'settings.fetchScheduleHint': { ja: 'この値はフィード取得の下限です。通知の送信間隔は変更しません。', en: 'This controls the minimum feed fetch interval only. Notification timing is unchanged.' },
  'settings.fetchScheduleMinutes': { ja: '分', en: 'minutes' },
  'settings.fetchScheduleInvalid': { ja: '1 から 240 の整数を入力してください', en: 'Enter a whole number between 1 and 240' },
...
const zhOverrides: Partial<Record<MessageKey, string>> = {
  ...
  'notifications.translateEnabled': '自动翻译正文预览为简体中文',
  'notifications.translateEnabledHint': '仅当正文预览不是简体中文时才翻译。标题和图片保持原样发送。',
  ...
  'notifications.previewBody': '标题：${feedName} · ${count} 条\n每篇文章包含：原文链接 / 精确到分钟的发布时间 / 正文原文 1 行 / 中文译文 1 行（仅在需要翻译时）/ 最多 3 张图片',
  ...
  'settings.fetchSchedule': '订阅抓取间隔',
  'settings.fetchScheduleDesc': '配置 RSS 订阅在后台抓取时使用的最小间隔',
  'settings.fetchScheduleMinInterval': '最小抓取间隔',
  'settings.fetchScheduleHint': '这里只控制订阅抓取下限，不会修改消息推送间隔。',
  'settings.fetchScheduleMinutes': '分钟',
  'settings.fetchScheduleInvalid': '请输入 1 到 240 之间的整数',
